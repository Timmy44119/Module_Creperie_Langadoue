    ...
    try {
      ...
      if (discounts != null) {
        ...
        this.logger.debug(
            ...
            discounts.get("lineSecondDiscountTypeSelect"),
            discounts.get("lineSecondDiscountAmount"));
        if ((saleOrderLine.getProduct().getInAti() != saleOrder.getInAti())
            && ((Integer) discounts.get("lineSecondDiscountTypeSelect")
                != PriceListLineRepository.AMOUNT_TYPE_PERCENT)) {
          response.setValue(
              "secDiscountAmount",
              saleOrderLineService.convertUnitPrice(
                  saleOrderLine.getProduct().getInAti(),
                  saleOrderLine.getTaxLine(),
                  (BigDecimal) discounts.get("lineSecondDiscountAmount")));
        } else {
          response.setValue("secDiscountAmount", discounts.get("lineSecondDiscountAmount"));
        }
        response.setValue("secDiscountTypeSelect", discounts.get("lineSecondDiscountTypeSelect"));
      }
